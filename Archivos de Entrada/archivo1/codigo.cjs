...
resultadoValor.Observador("modificado",funcion2(){
	si (Resultado < 0){
...
btn_formato.Observador("Clic", funcion3(){
		...
		Selecciona ( turno ){
			Caso 0 :
				si(contador == 0){
					...
					Valor1 : valor;					
					contador++;	
					mensaje("Valor1 :"+Valor1);
					''Imprimir("Contador era cero y valor1 es "+valor1);
				}
				sino{	
			...
			Caso 1 :
				...
				detener;
			Defecto :				
				mensaje("Error no es posible operar "+turno);
				detener;
		}
	}
	sino{
		...
		si(ContadorPunto==1){
			Selecciona ( turno ){
				...
				Caso 1 :
					si(contador == 1){
						ValorPunto : valor / 10;
						Valor2 : Valor2 + valorPunto;
						contador++;						
						mensaje("Valor2 :"+Valor1);
					}
					sino{
						ValorPunto : valor / ( 10 ^ contador);
						Valor2 : Valor2 + valorPunto;
...
funcion operar(operador){
	...
	si ( valor1 != "" && valor2 == "") {
		...
		contadorpunto : 0;		
		turno++;
		''imprimir("Ya hay operacion y es "+operadorAnterior);
		retornar "";
	}
